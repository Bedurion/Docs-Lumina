(() => {
  ...
  const commandGuides = {
    ...
    '/events configure': command('Universal event-template editor.', 'Creates server-specific event templates with custom slot names, Main capacity, Waitlist capacity and minimum attendance requirements.', 'An administrator opens the private editor, starts from Tank / DPS / Heal or No Classes, then creates or adjusts templates for that Discord server. Published events keep a snapshot of the template they were created with, so later edits affect only future events. This command is available only in Universal Edition.'),
    ...
    '/website transfer': command('Administrator-only Website credit transfer command.', 'Moves one published credit, or every published entry belonging to the same stable creator identity, to a Discord member or to Lumina as the guild owner.', 'Set destination to Member or Lumina. Member is required only for a Member destination; identify the publication with submission, or omit it inside its private review thread; use character to override the destination member’s default main-character credit; and enable all-content only when every published entry owned by the current creator should move together. Luminox records the approved creator operation and sends it through the secure publishing workflow while preserving each submission’s historical ownerId and private audit trail.'),
